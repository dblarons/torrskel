  ...
  describe "MetaInfoFile" $ do
    ...
    describe "extractTorrentMeta" $ do
      it "should extract a torrent file BDict into a record" $ do
        let expected =
              TorrentMeta
              {torrentMetaAnnounce = pack "foobar", torrentMetaInfo = metaInfo}
            encoded =
              BDict
                [ (pack "announce", BString $ pack "foobar")
    ...
    describe "extractMetaFile" $ do
      it "should extract a meta file BDict into a record" $ do
        let expected =
              MetaFile
              {metaFileLength = 100, metaFilePath = [pack "foo", pack "bar"]}
            encoded =
              BDict
                [ (pack "length", BInteger 100)
                , ( pack "path"
                  , BList [BString $ pack "foo", BString $ pack "bar"])
                ]
